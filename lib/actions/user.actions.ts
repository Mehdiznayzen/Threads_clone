...
export const updateUser = async ({ bio, image, name, path, userId, username } : ParamsUpdateUser): Promise<void> => {
    try {
        connectToDB()
...
export const getActivities = async (userId : string) => {
    try {
        ...
        const userThreads = await Thread.find({ author: userIdObjectId });
